...
#define LOG_TAG "android.hardware.light@2.0-service.xiaomi_cas"
...
Light::Light() {
    ...
    if (ReadFileToString(BLUE_ATTR(max_brightness), &buf) ||
        ReadFileToString(GREEN_ATTR(max_brightness), &buf) ||
...
void Light::applyNotificationState(const LightState& state) {
    ...
    if (state.flashMode == Flash::TIMED && state.flashOnMs > 0 && state.flashOffMs > 0) {
         ...
         * to fit.
         */
        int32_t pause_hi = state.flashOnMs - (kRampStepDuration * kBrightnessRamp.size() * 2);
        if (pause_hi < 0) {
            pause_hi = 0;
        }
        ...
        for (const auto& entry : colorValues) {
            ...
            WriteToFile(makeLedPath(entry.first, "delay_on"), static_cast<uint32_t>(state.flashOffMs));
